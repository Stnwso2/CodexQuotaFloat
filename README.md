...
- 右侧“技能”可把悬浮窗展开为 Skills 面板，按最近安装时间显示本机当前可发现的个人、插件和系统 Skills 及其用途摘要；面板支持滚动与手动刷新。
- 已安装 Codex Dream Skin 时，底部按钮可手动启用皮肤或热加载样式；托盘菜单可恢复官方样式。
- 皮肤操作只在点击后执行，不运行后台自动重启或自动注入守护。
...
- 双击悬浮窗：立即刷新。
- 点击右上角“技能”：展开或收起 Skills 面板；展开后可滚动浏览，右键菜单“显示可用 Skills”会重新扫描本机目录。
- 底部“启用皮肤”：启动 Codex Dream Skin，并按需重启一次 Codex。
- 底部“热加载”：皮肤启用后重新载入当前样式，不重启 Codex。
- 右键悬浮窗或系统托盘图标：“界面皮肤”中可查看状态、启用、热加载或恢复官方样式。
- 右键菜单还可刷新额度、切换最前显示或退出。

皮肤按钮调用本机 `%LOCALAPPDATA%\Programs\CodexDreamSkin\scripts` 中已安装的固定脚本；未安装 Dream Skin 时不会执行任何外部命令。恢复官方样式会重启一次 Codex。悬浮窗不会自动执行启用、恢复或重启操作。
